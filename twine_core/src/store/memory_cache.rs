...
impl<T: Resolver> MemoryCache<T> {
  pub fn new(resolver: T) -> Self {
    Self {
      cache: Arc::new(Mutex::new(MemoryStore::new())),
      resolver,
    }
  }
}
...
impl<T: Resolver> Resolver for MemoryCache<T> {
    ...
    Ok(
      self.resolver.strands().await?
        .and_then(|strand| async {
          let _ = self.cache.lock().await.save(strand.clone()).await;
          Ok(strand)
        })
        .boxed()
    )
  ...
  async fn resolve_cid<C: AsCid + Send>(&self, cid: C) -> Result<AnyTwine, ResolutionError> {
    let cid = cid.as_cid();
    let res = {
      self.cache.lock().await.resolve_cid(cid).await
    };
    if res.is_ok() {
      res
    } else {
      let twine = self.resolver.resolve_cid(cid).await?;
      let _ = self.cache.lock().await.save(twine.clone()).await;
      Ok(twine)
    }
  }
  ...
  async fn resolve_index<C: AsCid + Send>(&self, cid: C, index: u64) -> Result<Twine, ResolutionError> {
    let cid = cid.as_cid();
    let res = {
      self.cache.lock().await.resolve_index(cid, index).await
    };
    if res.is_ok() {
      res
    } else {
